...
from small_eval import evaluate
from log_parsing_zmj import template_extraction
import argparse
device = 'cuda:0'
random.seed(2023)
...
def fine_tuning_woCL(args):
    ...
    for idx,data_name in enumerate(files_SmallParse):
        ...
        logdata_train = multi_task_dataset(ori_data_path, [data_name], tokenizer, label_to_id,shot=shot)
        train_dataloader = DataLoader(logdata_train, shuffle=True, collate_fn=data_collator, batch_size=20)

        logdata_eval = multi_task_dataset(ori_data_path, [data_name], tokenizer, label_to_id, train_eval_test='eval',shot=shot)
        eval_dataloader = DataLoader(logdata_eval, shuffle=True, collate_fn=data_collator, batch_size=20)
        ...
        for epoch in range(num_train_epochs):
            ...
            if epoch % 5 == 0:
                best_metric = evaluate(metric, model, tokenizer, eval_dataloader, device,
                                       pad_to_max_length=False, id_to_label=id_to_label)
                print("Finish training, best metric: ")
                print(best_metric)

        sub_task_output_dir = task_output_dir
        os.makedirs(sub_task_output_dir, exist_ok=True)

        log_file = f'datasets_LP/{data_name}/{data_name}_2k.log_structured.csv'
        template_extraction(tokenizer, model, device, log_file, max_length=256,
                            model_name='roberta', shot=shot, dataset_name=data_name,
...
def train_MultiTask(args):
    ...
    label_to_id = {'o': 0, 'i-val': 1}
    id_to_label = {0:'o', 1:'i-val'}
    logdata_train = multi_task_dataset(ori_data_path, files_SmallParse, tokenizer, label_to_id)
    logdata_test = multi_task_dataset(ori_data_path, files_SmallParse, tokenizer, label_to_id,'test')
    data_collator = CustomDataCollator(
        tokenizer, pad_to_multiple_of=None
    )
    train_dataloader = DataLoader(logdata_train, shuffle=True, collate_fn=data_collator, batch_size=25)

    logdata_eval = multi_task_dataset(ori_data_path, files_SmallParse, tokenizer, label_to_id, train_eval_test='eval')
    ...
    task_output_dir = args.save + '/multi_task'
    os.makedirs(task_output_dir,exist_ok=True)
    ...
    for file in files_SmallParse:
        ...
        template_extraction(tokenizer, model, device, log_file, max_length=256,
                            ...
                            o_dir=task_output_dir, mode='fine-tuning')

def sequential_fine_tuning(args):

    files_SmallParse = args.permutation
    task_output_dir = args.save + '/sequential_fine-tuning'
    # task_output_dir = 'mix_systems_o1\sequential_fine-tuning'
    os.makedirs(task_output_dir, exist_ok=True)
    ...
    for idx,data_name in enumerate(files_SmallParse):
        ...
        logdata_eval = multi_task_dataset(ori_data_path, [data_name], tokenizer, label_to_id, train_eval_test='eval')
        eval_dataloader = DataLoader(logdata_eval, shuffle=True, collate_fn=data_collator, batch_size=20)
...
def inc_joint(args):
    ...
    for idx, data_name in enumerate(files_SmallParse):
        ...
        logdata_train = multi_task_dataset(ori_data_path, data_inc_list, tokenizer, label_to_id)
        train_dataloader = DataLoader(logdata_train, shuffle=True, collate_fn=data_collator, batch_size=25)

        logdata_eval = multi_task_dataset(ori_data_path, data_inc_list, tokenizer, label_to_id, train_eval_test='eval')
        eval_dataloader = DataLoader(logdata_eval, shuffle=True, collate_fn=data_collator, batch_size=25)
...
def sequential_er(args):
    ...
    for idx, data_name in enumerate(files_SmallParse):
        ...
        logdata_train = multi_task_dataset_buffer(ori_data_path, [data_name], tokenizer, label_to_id)
        ...
        train_dataloader = DataLoader(logdata_train, shuffle=True, collate_fn=data_collator, batch_size=16)
        logdata_eval = multi_task_dataset_buffer(ori_data_path, [data_name], tokenizer, label_to_id, train_eval_test='eval')
        eval_dataloader = DataLoader(logdata_eval, shuffle=True, collate_fn=data_collator, batch_size=16)
...
def simple_knowledge_distill(args):
    ...
    for idx, data_name in enumerate(files_SmallParse):
        if idx == 0:
            model_student = RobertaForTokenClassification.from_pretrained(model_path)
            model_student.to(device)
            tokenizer.model_max_length = model_student.config.max_position_embeddings - 2
        else:
            model_teacher = RobertaForTokenClassification.from_pretrained(model_path)
        ...
        if idx == 0:
            for epoch in range(num_train_epochs):
                ...
                if epoch % 5 == 0:
                    best_metric = evaluate(metric, model_student, tokenizer, eval_dataloader, device,
                                           pad_to_max_length=False, id_to_label=id_to_label)
                    print("Finish training, best metric: ")
                    print(best_metric)
        else:
        ...
        for file in files_SmallParse:
            log_file = f'datasets_LP/{file}/{file}_2k.log_structured.csv'
            template_extraction(tokenizer, model_student, device, log_file, max_length=256,
                                model_name='roberta', shot=shot, dataset_name=file,
                                o_dir=sub_task_output_dir, mode='fine-tuning')
